...
class Client_Email:
    def __init__(self, email_id, cog_load, obv_level, log=True):
        ...
        self.log_file.write('COGLOAD ' + cog_load + "\n")
        self.log_file.write('START ' + str(datetime.now()) + '\n')
        self.log_file.write('SIGNAL ' + str(obv_level) + '\n')

        self.obv_level = obv_level
        ...
        while not self.finished_flag.isSet():
            ...
            if len(new_email_fetch[0].split()) - len(old_email_fetch[0].split()) > 0:
                ...
                if self.client_session != None:
                    try:
                        self.client_session.signal(self.obv_level)
                    except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-check_freq', action="store", dest="check", type=float, default=2)
    parser.add_argument('-obviousness', action="store", dest="obv", type=int, default=1)
    ...
    if args.id != None:
        ...
        print "Incorrect arguments. Specify id, and mins, optionally check_freq and obviousness, or use python command line."